...
class LibraryCell: RoundCornerCell {
    // MARK: - View
    ...
    func populate(with library: Library) {
        indicatorView.backgroundColor = library.occupiedPercentage.color
        titleLabel.text = library.name
        occupiedLabel.text = library.occupiedSeats.readable
        availableLabel.text = library.availableSeats.readable
        progressView.progress = Float(library.occupiedPercentage)
        progressView.progressTintColor = library.occupiedPercentage.color
        contentView.accessibilityLabel = "\(library.name) \("available".localized()) \(library.availableSeats) \("occupied".localized()) \(library.occupiedSeats)"
    }
}
