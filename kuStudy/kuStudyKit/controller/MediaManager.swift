...
class MediaManager {
    ...
    func media(for library: LibraryType) -> Media? {
        // Snapshot
    ...
    func preferredMedia(for library: LibraryType) -> [Media] {
        let timeframe = Media.Timeframe.suitableTimeframeForNow()
        let result = media(for: library,
                          artist: nil,
                          mediaType: .photo,
